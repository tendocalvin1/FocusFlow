    ...
    ? [
        {
          ...
          value: summary.total_focus_hours,
          icon: Clock,
          color:
    ...
    return (
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[0, 1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-24 animate-pulse rounded-2xl border border-slate-200/80 bg-slate-100/80 dark:border-slate-800 dark:bg-slate-900"
          />
        ))}
      </div>
    );
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {kpis.map((kpi, idx) => {
        ...
        return (
          <Card
            key={idx}
            className="rounded-2xl border border-slate-200/80 bg-white/90 shadow-2xs dark:border-slate-800 dark:bg-slate-900/90"
          >
            <CardContent className="flex items-center justify-between p-5">
              ...
              <div
                className={`flex h-11 w-11 items-center justify-center rounded-xl ${kpi.color}`}
              >
